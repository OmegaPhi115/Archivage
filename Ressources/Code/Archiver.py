...
def Archiver(safe_mode = True):
    ...
    if intype != 0:
        if 1:
                ...
                try:
                    ...
                    if intype == 1:
                        if datea <= 1970:
                            ...
                            if dino == 5:
                                os.startfile(
                                    '\\Program Files (x86)\\Python Programs\\Data\\Archivage\\téléchargement.png')
                        ...
                        try:
                            ...
                            print("")
                            print(
                                "Les ordinateurs n'existait pas encore, Veuiller entrer une date supérieure a l'epoch (1970)")
                            print("")
                except ValueError:
                    print("")
            ...
            toasteur(notoast, "Archivage commencé", "Archivage des donnés de " + str(path),
                     "/Program Files (x86)/Python Programs/Data/Archivage/454.ico", 10)
            ...
            toasteur(notoast, "Archivage terminé",
                     "Sur " + str(filecountercheck) + " fichiers, " + str(filecounter) + " ont été déplacés",
                     "/Program Files (x86)/Python Programs/Data/Archivage/transparent-green-checkmark-hi.ico", 5)
            ...
            if notoast == 0:
                if noperm != 0:
                    ...
                    toaster.show_toast("Pas de Permition",
                                       str(
                                           noperm) + " fichiers n'ont pas put êtres deplacés. Plus d'informations sont disponible dans le fichier Info.txt de l'archive",
                ...
                if noperm == 0:
                    if copyfail == 0:
                        toaster = ToastNotifier()
                        toaster.show_toast("Tout les fichiers ont bien été déplacés", " ",
                                           icon_path="/Program Files (x86)/Python Programs/Data/Archivage/transparent-green-checkmark-hi.ico",
                                           duration=5)
                ...
                if copyfail != 0:
                    ...
                    toaster.show_toast("Probleme de copie",
                                       str(
                                           copyfail) + " fichiers n'ont pas put êtres deplacés. Les fichiers sont deja dans l'archive, il doivent être deplacés manuellement. Plus d'informations sont disponible dans le fichier Info.txt de l'archive",
                                       icon_path="/Program Files (x86)/Python Programs/Data/Archivage/attention_PNG60.ico",
                                       duration=20)
